...
class DashboardHomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent crawl runs (last 10) — crawler pipeline only
        recent_crawl_runs = CrawlRun.objects.order_by("-started_at")[:10]

        # Crawler summary from CrawlRun (last run stats for dashboard card)
        crawler_last_run = CrawlRun.objects.order_by("-started_at").first()
        crawler_last_stats = crawler_last_run.stats if crawler_last_run else None
        crawler_services = []
        if crawler_last_stats:
            svc = dict((crawler_last_stats or {}).get("services_detected") or {})
        ...
        context.update(
            {
                ...
                # Crawler summary (CrawlRun-based only)
...
class RunDetailView(LoginRequiredMixin, DetailView):
    """View scrape run details (scraper only; crawler runs use CrawlRunDetailView)."""
...
class CrawlRunDetailView(LoginRequiredMixin, DetailView):
    """View crawler run details with per-domain results."""
    ...
    def get_context_data(self, **kwargs):
        ...
        # Per-domain results (CrawlRunDomainResult linked to this CrawlRun)
        state_counts = (
            CrawlRunDomainResult.objects.filter(crawl_run_id=run.id)
            .values("state")
            .annotate(count=Count("id"))
        )
        counts = {row["state"]: int(row["count"] or 0) for row in state_counts}
        processed = CrawlRunDomainResult.objects.filter(
            crawl_run_id=run.id, processed_at__isnull=False
        ).count()
        failed = CrawlRunDomainResult.objects.filter(
            crawl_run_id=run.id, state="failed"
        ).count()
        context["crawler_progress"] = {
            "expected": int(run.domains_queued or 0),
            "processed": int(processed or 0),
            "failed": int(failed or 0),
            "states": counts,
        }
        context["crawler_failures"] = list(
        ...
        context["domain_results_page_obj"] = paginator.get_page(dpage)

        services_detected = dict((run.stats or {}).get("services_detected") or {})
        context["services_detected_items"] = sorted(
