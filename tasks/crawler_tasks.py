...
def crawl_domain_task(self, domain_id: int, run_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        analyze_domain_task.delay(domain_id=domain_id, crawl_data=data, run_id=run_id)
        return {"domain_id": domain_id, "pages_crawled": int(data.get("pages_crawled") or 0)}
    except Exception as exc:
...
def score_and_create_prospect_task(self, domain_id: int, run_id: int) -> dict[str, Any]:
    ...
    # Update run stats (best-effort, idempotent-ish increments)
    try:
        with transaction.atomic():
            ...
            if created:
                stats["prospects_created"] = int(stats.get("prospects_created") or 0) + 1
                run.created_leads = int(run.created_leads or 0) + 1
            if updated:
                stats["prospects_updated"] = int(stats.get("prospects_updated") or 0) + 1
                run.updated_leads = int(run.updated_leads or 0) + 1
            if label == "high":
                stats["high_score_leads"] = int(stats.get("high_score_leads") or 0) + 1
            if label == "medium":
                stats["medium_score_leads"] = int(stats.get("medium_score_leads") or 0) + 1
            ...
            run.stats = stats
            run.save(update_fields=["stats", "created_leads", "updated_leads", "updated_at"])
    except Exception:
        logger.debug("Failed to update ScrapeRun stats for run_id=%s", run_id, exc_info=True)
